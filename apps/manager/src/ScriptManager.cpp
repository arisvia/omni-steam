...
std::string SanitizeCommentLine(std::string value) {
    std::replace(value.begin(), value.end(), '\r', ' ');
    std::replace(value.begin(), value.end(), '\n', ' ');
    return value;
}

bool PathIsUnder(const fs::path& child, const fs::path& parent) {
...
std::string ScriptManager::GenerateLuaScript(const UnlockGameSpec& spec) {
    ...
    oss << "-- Game: " << SanitizeCommentLine(spec.gameName) << "\n";
    ...
    if (!mainKey.empty()) {
        ...
        oss << "addappid(" << spec.appId << ", 0, \"" << EscapeLuaString(mainKey) << "\")\n\n";
    } else {
    ...
    for (const auto& [depotId, keyHex] : spec.depotKeys) {
        if (depotId != spec.appId && dlcSet.find(depotId) == dlcSet.end() && !keyHex.empty()) {
            ...
            oss << "addappid(" << depotId << ", 0, \"" << EscapeLuaString(keyHex) << "\")\n";
    ...
    if (!spec.dlcAppIds.empty()) {
        ...
        for (uint32_t dlcId : spec.dlcAppIds) {
            ...
            if (it != spec.depotKeys.end() && !it->second.empty()) {
                oss << "addappid(" << dlcId << ", 0, \"" << EscapeLuaString(it->second) << "\")\n";
            } else {
                oss << "addappid(" << dlcId << ")\n";
            }
    ...
    if (!spec.accessToken.empty()) {
        oss << "-- Access Token\n";
        oss << "addtoken(" << spec.appId << ", \"" << EscapeLuaString(spec.accessToken) << "\")\n\n";
    }

    // 5. Custom Manifest ID if present
    if (!spec.customManifestId.empty()) {
        oss << "-- Manifest Pin\n";
        oss << "setManifestid(" << spec.appId << ", \"" << EscapeLuaString(spec.customManifestId) << "\")\n\n";
    }

    // 6. Denuvo & App Ownership Tickets (All-In-One)
    if (!spec.appTicketHex.empty()) {
        oss << "-- Denuvo / App Ownership Ticket\n";
        oss << "setAppTicket(" << spec.appId << ", \"" << EscapeLuaString(spec.appTicketHex) << "\")\n\n";
    }
    if (!spec.eTicketHex.empty()) {
        oss << "-- Denuvo Encrypted Ticket (ETicket)\n";
        oss << "setETicket(" << spec.appId << ", \"" << EscapeLuaString(spec.eTicketHex) << "\")\n\n";
    }
...
bool ScriptManager::ToggleScript(const std::string& filePath, bool enable) {
    try {
        if (!IsManagedScriptPath(filePath)) {
            spdlog::warn("ScriptManager: Rejected toggle outside managed Lua directories: {}", filePath);
            return false;
        }
...
bool ScriptManager::DeleteScript(const std::string& filePath) {
    try {
        if (!IsManagedScriptPath(filePath)) {
            spdlog::warn("ScriptManager: Rejected delete outside managed Lua directories: {}", filePath);
            return false;
        }
