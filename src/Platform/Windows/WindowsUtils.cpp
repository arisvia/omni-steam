...
std::string Hash::Sha256(const std::vector<uint8_t>& data) { return ""; }
std::string Hash::Sha256File(const std::string& filePath) { return ""; }
std::string Hash::Md5(const std::vector<uint8_t>& data) { return ""; }
...
bool CredentialStore::WriteTicket(uint32_t appId, const std::string& ticketName, const std::string& hexValue) {
    ...
    if (RegCreateKeyExA(HKEY_CURRENT_USER, subKey.c_str(), 0, nullptr, 0, KEY_SET_VALUE, nullptr, &hKey, nullptr) == ERROR_SUCCESS) {
        RegSetValueExA(hKey, ticketName.c_str(), 0, REG_SZ, reinterpret_cast<const BYTE*>(hexValue.c_str()), static_cast<DWORD>(hexValue.length() + 1));
        RegCloseKey(hKey);
        return true;
    }
    return false;
...
std::string CredentialStore::ReadTicket(uint32_t appId, const std::string& ticketName) {
    ...
    if (RegOpenKeyExA(HKEY_CURRENT_USER, subKey.c_str(), 0, KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS) {
        ...
        DWORD type = REG_SZ;
        if (RegQueryValueExA(hKey, ticketName.c_str(), nullptr, &type, reinterpret_cast<LPBYTE>(buf), &bufSize) == ERROR_SUCCESS) {
            RegCloseKey(hKey);
