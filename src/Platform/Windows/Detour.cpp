...
bool Detour::Attach(void** ppPointer, void* pDetour) {
    ...
    if (rv != 0) {
        spdlog::error("Windows funchook_prepare failed: {}", funchook_error_message(g_funchook));
        return false;
    }
    return true;
}
...
bool Detour::CommitTransaction() {
    ...
    if (rv != 0) {
        spdlog::error("Windows funchook_install failed: {}", funchook_error_message(g_funchook));
